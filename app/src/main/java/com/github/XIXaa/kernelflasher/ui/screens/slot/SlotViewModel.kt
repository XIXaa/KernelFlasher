package com.github.XIXaa.kernelflasher.ui.screens.slot
...
import com.github.XIXaa.kernelflasher.common.PartitionUtil
import com.github.XIXaa.kernelflasher.common.extensions.ByteArray.toHex
import com.github.XIXaa.kernelflasher.common.extensions.ExtendedFile.inputStream
import com.github.XIXaa.kernelflasher.common.extensions.ExtendedFile.outputStream
import com.github.XIXaa.kernelflasher.common.types.backups.Backup
import com.github.XIXaa.kernelflasher.common.types.partitions.Partitions
...
class SlotViewModel(
    ...
    private fun _getKernel(context: Context) {
        ...
        if (kernel.exists()) {
            val result =
                Shell.cmd("strings kernel | grep -E -m1 'Linux version.*#' | cut -d\\  -f3-")
                    .exec().out
        ...
        for (partitionName in PartitionUtil.PartitionNames) {
            if (_backupPartitions[partitionName] == true) {
                ...
                if (blockDevice != null) {
                    ...
                    val hash =
                        backupPartition(blockDevice, destination.getChildFile("$partitionName.img"))
                    if (hash != null) {
    ...
    private fun createBackupDir(context: Context, now: String): ExtendedFile {
        @SuppressLint("SdCardPath")
        val externalDir = fileSystemManager.getFile("/sdcard/Flash_data")
    ...
    fun backup(context: Context) {
        launch {
            ...
            val backup =
                Backup(now, "raw", currentKernelVersion!!, sha1, null, hashes, hashAlgorithm)
        ...
        launch {
            ...
            if (source != null) {
                ...
                jsonFile.outputStream().use {
                    it.write(
                        indentedJson.encodeToString(backup).toByteArray(Charsets.UTF_8)
                    )
                }
    ...
    private fun _copyFile(context: Context, currentBackup: String, filename: String) {
        ...
        @SuppressLint("SdCardPath")
        val externalDir = File("/sdcard/Flash_data")
        ...
        try {
            if (zip.exists()) {
                ...
                val result = Shell.Builder.create()
                    .setFlags(Shell.FLAG_MOUNT_MASTER or Shell.FLAG_REDIRECT_STDERR).build()
                    .newJob().add("F=$files Z=\"$zip\" /system/bin/sh $flashScript").to(flashOutput)
                    .exec()
                if (result.isSuccess) {
